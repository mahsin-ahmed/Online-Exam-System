...
if($isAuthenticate != 1){
	...
	include('db.php');
	$sql = "SELECT Id,Description,QuizLevelId,SecPerQuestion,Title FROM questionmaster";
	if ($result = mysqli_query($con, $sql)) {
		// Fetch one and one row
		$i = 0;
		$listQuestionMaster = array();
		while ($row = mysqli_fetch_row($result)) {
			$oQuestionMaster['Id'] = $row[0];
			$oQuestionMaster['Description'] = $row[1];
			$oQuestionMaster['QuizLevelId'] = $row[2];
			$oQuestionMaster['SecPerQuestion'] = $row[3];
			$oQuestionMaster['Title'] = $row[4];
			$listQuestionMaster[] = $oQuestionMaster;
			$i++;
		}
		listToJson($listQuestionMaster);
	...
	if (isset($_GET['id']) && $_GET['id']!="") {
		include('db.php');
		$id = $_GET['id'];
		$sql = "SELECT Id,Description,QuizLevelId,SecPerQuestion,Title FROM questionmaster WHERE Id=$id";
		$result = mysqli_query($con, $sql);
		if(mysqli_num_rows($result)>0) {
			$row = mysqli_fetch_array($result);
			$Id=$row['Id'];
			$Description=$row['Description'];
			$QuizLevelId=$row['QuizLevelId'];
			$SecPerQuestion=$row['SecPerQuestion'];
			$Title=$row['Title'];
			objectToJson($Id, $Description, $QuizLevelId,$SecPerQuestion,$Title);
			mysqli_close($con);
		} else {
...
function objectToJson($Id, $Description, $QuizLevelId,$SecPerQuestion,$Title) {
	$response['Id'] = $Id;
			$response['Description']=$Description;
			$response['QuizLevelId']=$QuizLevelId;
			$response['SecPerQuestion']=$SecPerQuestion;
